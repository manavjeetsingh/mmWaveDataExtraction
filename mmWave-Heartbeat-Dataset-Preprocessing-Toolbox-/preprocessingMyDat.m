...
function P = getRadarParams()
    ...
    P.c        = 3e8;
    P.slope    = 65.998e12;
    P.startFreq = 77.396e9;
    P.lambda    = P.c / P.startFreq;
    
    % Chirp / frame timing
    P.framePeriod = 2.5*1.6e-4;
    ...
    % File
    P.binIdx = 1;
    P.dataSubDir = fullfile('mydat','aditya_nohoodie1');
end
...
function adcData = loadADCData(P)
    ...
    % file    = sprintf('samir_nomovement_nobreathing%d.bin', P.binIdx);
    file    = sprintf('aditya_tshirt_nomovement_nobreathing%d.bin', P.binIdx);
...
function process_adc = selectRXAndChirps(LVDS, P)
    ...
    numFrames = numChirps / P.chirpsPerFrame;
    process_adc = retVal(:,1:P.chirpsPerFrame:end);   % take first chirp per frame
