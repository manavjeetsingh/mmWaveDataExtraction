...
function car_display(figureTab, tracker, numRows, xscale, zone_pos, frameIdx)
...
hold on;
    ...
    if (frameFlag == 1)
        %Front row
        for i = 1:3        
            subplot('Position', zone_pos(i,:));
            cla(subplot('Position', zone_pos(i,:)));
            ...
            if (tracker(i).state)
               if(i==1)
                   imshow(intruder, 'Border','tight', 'InitialMagnification', 'fit');
               else
